...
def assign_cluster_img(k, v_x, v_c):
    ...
    """
    #cette fonction enregistre les positions des points des clusters a la difference de celle implementé dans
    #les autres fichiers.
    i, j = 0, 0
    n = len(v_x)
    m_clusters = [[] * n for j in range(k)]#initialise une matrice de k elements chaque ligne  i contient les points du ieme cluster
    while(j < n):
    	dis_min = np.inf
    	i = 0
    	centroide = i
...
def k_means_im(k, conv, v_x):
	...
	retourne la mtrice de l image modifie avec les nouvelles couleurs 
...
if __name__ == "__main__":
	img = load_image('landscape_200px.png')
