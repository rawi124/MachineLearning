...
def generate_trainset():
    ...
    """
    m_x, v_label = ds.make_blobs(
        n_samples=400, centers=2, n_features=2, cluster_std=[3, 3], random_state=42)
    return m_x, v_label
...
def generate_testset():
    ...
    RETURN
            ...
            v_label : vector of label (0 or 1)
    """
    m_x, v_label = ds.make_blobs(n_samples=200, centers=2, n_features=2, cluster_std=[
                                 3.2, 3.2], random_state=42)
    return m_x, v_label
...
def nearest_neighbor(k, dataset, x):
    ...
    i = 0
    while i < len(dataset):
        dist.append([dist_euc(dataset[i], x), i])
    # on calcule la distance de chaque points par rapport au point x
        i += 1
    dist = sorted(dist,)  # on ordonne notre liste de distance
    indices = dist[1:k+1]
# on récupere les indices des k plus proches voisins de x
...
def dominant_label(voisins, y):
    """
    qui retourne le label majoritaire dans un set
de données voisins (qui est renvoyé par la fonction precedente)
    """
    labels = []
    s = 0
...
def accuracy_score(y_true, y_pred):
    """
    calculer le nombre d instances bien classé
entre y_true renvoyé par la premiere fonction de genration 
...
def knn(train_data, v_label, test_data, k):
    """
    fait appel aux differentes methodes 
    pour fournir le y_predit
    """
    y_pred = []
    for el in test_data:
...
if __name__ == "__main__":
    ...
    for k in l_k:
        ...
        print("accuray score ", acc, "recall score ",
              re, "precision score ", pre)
